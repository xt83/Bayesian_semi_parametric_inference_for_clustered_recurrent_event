...
observed<- matrix(0,nrow=n,ncol=300)
...
sigma_true<-rgamma(n,shape=7.5,scale=1)
...
t_0<-max(observed[,5])
time_1<-c(0,t_0/5,2*t_0/5,3*t_0/5,4*t_0/5,t_0)
for (i in 1:n){
  if (y_true[i]==0){
    ...
    t_max<- observed[i,5]
    lambda<- function(t){
      lambda_true<-2*ifelse(t>=time_1[1]&t<=time_1[2],1,0)+2.3*ifelse(t>time_1[2]&t<=time_1[3],1,0)+2.1*ifelse(t>time_1[3]&t<=time_1[4],1,0)+2.4*ifelse(t>time_1[4]&t<=time_1[5],1,0)+1.9*ifelse(t>time_1[5]&t<=time_1[6],1,0)
      val<- gamma_true[i]*lambda_true*exp(betar_true%*%zr[i,]+mu_hos_true[observed[i,2]])
      return (val)
    }
...
#for (w in 1:n){
    ...
    #}
#}
#time_q<-quantile(time_t)
#time_q<-c(0,time_q)
time_q<-time_1
lambda_n<- matrix(0,nrow=mcmc_samples,ncol=5)
lambda_n[1,]<- c(2,2.3,2.1,2.4,1.9)
metrop_var_lambda_n<- rep(0.01,times=5)
...
save(mcmc_samples,betad, betad_true,betar, betar_true,sigma,sigma_true,y,y_true,y_index,eta,eta_true,tao,zeta,file = paste0("n_combn_sim_06_all_",n,"_", seed,"_partly_overlap_strong_all_logistic_larger_variance_f5", ".rdata")) 
